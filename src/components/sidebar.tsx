...
function isServiceSectionPath(pathname: string) {
  if (!pathname.startsWith("/servis")) {
    return false;
  }

  return pathname !== "/servis/hizli-kabul";
}
...
export function Sidebar() {
  ...
  const serviceSectionPath = isServiceSectionPath(pathname);
        ...
        <nav className="flex-1 space-y-2 overflow-y-auto pr-1">
          {items.map((item) => {
            const active =
              ...
              (item.href === "/servis" && serviceSectionPath);
